...
class Trainer:
    ...
    def fit(self, model: GenericModel, train_loader, val_loader=None, test_loader=None):
    ...
    def test(self, model: GenericModel, test_loader, is_load=True):
        # Setup
        if torch.cuda.is_available():
            device = torch.device('cuda')
        else:
        ...
        if os.path.exists(os.path.join(model._log_path, model.cfg.benchmark)):
            shutil.rmtree(os.path.join(model._log_path, model.cfg.benchmark),ignore_errors=True)
