...
from pytorch3d.ops import packed_to_padded, ball_query, knn_points
...
def batch_knn_kpconv_gpu(queries, supports, q_batches, s_batches, max_neighbors):
    """
    Computes knn for a batch of queries and supports
    :param queries: (N1, 3) the query points
    :param supports: (N2, 3) the support points
    :param q_batches: (B) the list of lengths of batch elements in queries
    :param s_batches: (B)the list of lengths of batch elements in supports
    ...
    dist, idx, _ = knn_points(queries_padded, supports_padded,
                     q_batches, s_batches, K=max_neighbors, return_nn=False)  # (N_clouds, N_pts, K)
...
class PreprocessorGPU(torch.nn.Module):
    ...
    def forward(self, pts: List[torch.Tensor]):
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            # Get all blocks of the layer unless pool or strided
            if not ('pool' in block or 'strided' in block):
                layer_blocks += [block]
                # continue
                if block_i < len(config.architecture) - 1 and not (
